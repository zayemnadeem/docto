...
SECRET_KEY = os.getenv("SECRET_KEY", "your-super-secret-secret-key-that-is-long-enough")
ALGORITHM = os.getenv("ALGORITHM", "HS256")
ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", "1440"))

pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")
# We use standard JSON payload for login, but declare tokenUrl just in case
...
def get_current_user_logic(token: str, db: Session):
    ...
    if token_data.role == "patient":
        ...
        user = db.query(models.Doctor).filter(models.Doctor.id == token_data.user_id).first()
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    user, role = get_current_user_logic(token, db)
    return user
